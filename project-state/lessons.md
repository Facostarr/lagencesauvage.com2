...
- **Charger `copywriting` + `humanizer` AVANT de rédiger, pas après (lesson 2026-07-16)** : sur la session 2 articles gouvernance/outils, ces 2 skills n'ont été chargées qu'après la rédaction complète, via un audit rétroactif en Workflow (43 agents, ~8 min, vérification adversariale). L'audit a fonctionné et a rattrapé de vrais tics IA (métaphore grandiloquente, généralisations non sourcées type "la plupart des dirigeants..."), mais c'est un filet de sécurité coûteux comparé à charger les 2 skills dès le début du brief, avant d'écrire le premier mot. Réflexe à prendre systématiquement en plus de `seo-blog-writer` + `geo-optimization`.
- **Angle "état de l'art" générique = signal de cannibalisation probable** : avant d'accepter un brief type "article état de l'art sur X", lister d'abord les articles existants sur X (le cluster agents IA en avait déjà 7). Si le sujet est déjà largement couvert, proposer un angle plus étroit et frais (actualité réglementaire, framework original, données jamais utilisées) plutôt que la synthèse générique demandée — la synthèse générique dilue le signal SEO au lieu de le renforcer.
...
- **Front matter `date` au jour J = risque d'exclusion silencieuse (lesson 2026-07-16)** : Hugo compare la `date` du front matter à l'heure UTC courante (pas l'heure locale) pour décider si une page est "future" ; sans `--buildFuture`, une page future est exclue du build SANS ERREUR (juste absente du sitemap/listing, aucun warning). Piège concret : à minuit locale passée mais UTC pas encore le jour J (ex. minuit à Paris = 22h/23h UTC la veille), une `date: <jour J>` fraîchement mise est déjà "future" pour Hugo. Réflexe avant tout push d'article publié "aujourd'hui" : builder en local (`/c/tmp/hugo158/hugo.exe --destination <tmp> --noBuildLock`, SANS `--buildFuture`) et vérifier que le slug apparaît bien dans `<dest>/blog/<slug>/` avant de committer. Si absent, la date est trop optimiste : reculer d'un jour.
