// @ts-check
/**
 * compute_budget.js — Port JS du moteur de calcul Python.
 *
 * Doit produire EXACTEMENT les mêmes résultats que scripts/compute_budget.py
 * pour tout (idcc, tefen_code, naf_fallback_slug). Validé par
 * scripts/cross_validate.py.
 *
 * Conformité : consensus dual-LLM S2 (2026-05-23). Port sémantique (pas
 * "ligne à ligne") avec idiomes JS-safe (?. et ??), null explicite jamais
 * undefined, tri des dispositifs avant retour.
 */
...
/**
 * Somme des planchers garantis. Retourne null si aucun plancher chiffrable.
 * @param {DispositifActive[]} items
 * @param {boolean} inclusOnly
 * @returns {number|null}
 */
export function _sumPlanchers(items, inclusOnly = true) {
  const filtered = items
    .filter((d) => d.plancher_garanti_eur !== null && (!inclusOnly || d.inclus_dans_budget_chiffre))
    .map((d) => /** @type {number} */ (d.plancher_garanti_eur));
...
export function computeBudget({
  ...
  const budgetMax = _sumPlanchers(itemsMax, true);
