---
title: Simulateur budget formation Hospitalisation privée (IDCC 2264) 2026
seo_title: 'Hospitalisation privée : OPCO Santé, budget formation 2026'
description: 'Calculez votre budget formation 2026 pour la convention Hospitalisation privée (OPCO Santé, IDCC 2264, 2104). Plafonds PDC par tranche d''effectif, dispositifs activables, sources officielles. Simulation en 30 secondes.'
date: 2026-05-23
lastmod: 2026-05-23
layout: "branche-fiche"
robots: "index, follow"
canonical: "/simulateur-opco/branches/opco-sante-hp/"
ogImage: "/assets/images/logo-agence-sauvage.svg"
branche_slug: opco-sante-hp
branche_nom: Hospitalisation Privée et Thermalisme
branche_nom_court: Hospitalisation privée
branche_opco_slug: opco-sante
branche_opco_label: OPCO Santé
branche_idcc_primary: 2264
branche_idccs: 2264, 2104
keywords:
  - "budget formation Hospitalisation privée"
  - "OPCO Hospitalisation privée"
  - "IDCC 2264"
  - "simulateur IDCC 2264"
  - "OPCO Santé Hospitalisation privée"
  - "financement formation 2026"
faq:
  - question: "Comment savoir si mon entreprise relève de la convention Hospitalisation Privée et Thermalisme ?"
    answer: "Votre IDCC (Identifiant De Convention Collective) est indiqué sur vos bulletins de paie et sur votre Déclaration Sociale Nominative (DSN). Si vous ne le connaissez pas, notre <a href=\"/simulateur-opco/\">simulateur identifie automatiquement votre convention</a> depuis votre raison sociale ou votre SIREN, en interrogeant les bases publiques DINUM et siret2idcc. Pour la branche Hospitalisation Privée et Thermalisme, l'IDCC est 2264, 2104."
  - question: "La contribution conventionnelle est-elle obligatoire pour bénéficier du financement OPCO Santé ?"
    answer: "Oui. La branche Hospitalisation Privée et Thermalisme prévoit une <strong>contribution conventionnelle</strong> versée à OPCO Santé en plus de la contribution légale (0,55% sous 11 salariés, 1% au-delà). Cette cotisation finance les dispositifs branche (AFEST, bonus thématiques, parcours stratégique). Vérifiez votre situation auprès de votre expert-comptable ou directement auprès de OPCO Santé. Notre simulateur le précise dans le récapitulatif de votre simulation."
  - question: "Quel budget formation 2026 puis-je espérer pour la branche Hospitalisation Privée et Thermalisme ?"
    answer: "Le budget dépend de votre <strong>tranche d'effectif</strong> (TPE moins de 11 salariés, PME de 11 à 49, ETI au-delà de 50) et des dispositifs activés. La fourchette typique va de <strong>2 000 € à 15 000 € par an</strong> pour le seul Plan de Développement des Compétences, hors bonus thématiques et parcours stratégique. Pour un calcul précis basé sur les barèmes officiels OPCO Santé 2026, <a href=\"/simulateur-opco/\">lancez le simulateur en 30 secondes</a>."
---

Vous dirigez une entreprise relevant de la convention **Hospitalisation privée** (IDCC 2264, 2104) ? Votre budget formation 2026 est financé par OPCO Santé, avec des barèmes spécifiques à votre branche. Cette page récapitule les plafonds officiels, les dispositifs activables (PDC, AFEST, bonus thématiques) et les démarches pour mobiliser ces fonds.

[**Calculez votre budget OPCO Santé 2026 en 30 secondes →**](/simulateur-opco/)

<div class="not-prose my-8 rounded-2xl border-2 border-indigo-100 bg-indigo-50/30 p-5 sm:p-6">
  <h2 class="text-sm font-bold uppercase tracking-wide text-indigo-700 mb-4">Carte d'identité de la branche</h2>
  <dl class="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
    <div>
      <dt class="text-slate-500 mb-1">OPCO de rattachement</dt>
      <dd class="text-slate-900 font-semibold"><a href="/simulateur-opco/opco-sante/" class="text-indigo-700 hover:text-indigo-900 underline">OPCO Santé</a></dd>
    </div>
    <div>
      <dt class="text-slate-500 mb-1">IDCC(s) couvert(s)</dt>
      <dd class="text-slate-900 font-semibold">2264, 2104</dd>
    </div>
    <div>
      <dt class="text-slate-500 mb-1">Contribution conventionnelle</dt>
      <dd class="text-slate-900">Oui (cotisation conventionnelle obligatoire)</dd>
    </div>
    <div>
      <dt class="text-slate-500 mb-1">Subrogation OPCO</dt>
      <dd class="text-slate-900">À confirmer auprès de l'OPCO</dd>
    </div>
    <div class="sm:col-span-2">
      <dt class="text-slate-500 mb-1">Année d'application des barèmes</dt>
      <dd class="text-slate-900"><time datetime="2026">2026</time> · Données vérifiées le 2026-05-18</dd>
    </div>
  </dl>
</div>

## Plafonds PDC OPCO Santé 2026 par tranche d'effectif

Voici les plafonds officiels du Plan de Développement des Compétences pour la convention Hospitalisation Privée et Thermalisme en 2026, ventilés selon votre tranche d'effectif. Le pivot **50 salariés** marque la bascule réglementaire entre PME et ETI.

<div class="not-prose my-8 overflow-x-auto rounded-xl border border-slate-200">
  <table class="w-full text-left">
    <caption class="sr-only">Plafonds PDC OPCO Santé 2026 par tranche d'effectif</caption>
    <thead class="bg-slate-100">
      <tr>
        <th class="px-4 py-3 text-xs font-bold uppercase tracking-wide text-slate-600">Catégorie</th>
        <th class="px-4 py-3 text-xs font-bold uppercase tracking-wide text-slate-600">Effectif</th>
        <th class="px-4 py-3 text-xs font-bold uppercase tracking-wide text-slate-600">Plafond annuel</th>
        <th class="px-4 py-3 text-xs font-bold uppercase tracking-wide text-slate-600">Plafond horaire</th>
        <th class="px-4 py-3 text-xs font-bold uppercase tracking-wide text-slate-600">Taux PEC</th>
      </tr>
    </thead>
    <tbody>
    <tr class="border-t border-slate-200">
      <td class="px-4 py-3 text-sm font-semibold text-indigo-700 whitespace-nowrap">TPE-PME</td>
      <td class="px-4 py-3 text-sm text-slate-700">1 à 49 salariés</td>
      <td class="px-4 py-3 text-sm text-slate-900 font-semibold whitespace-nowrap">2 500 €</td>
      <td class="px-4 py-3 text-sm text-slate-700 whitespace-nowrap">sur projet</td>
      <td class="px-4 py-3 text-sm text-slate-700">—</td>
    </tr>
    <tr class="bg-slate-50/50 border-t border-slate-100">
      <td colspan="5" class="px-4 py-2 text-xs text-slate-600 italic">Coûts pédagogiques 100% + rémunération max 1 SMIC + frais</td>
    </tr>
    </tbody>
  </table>
</div>

> ℹ️ **Prise en charge** : coût pédagogique + frais annexes (repas, hébergement, déplacement).

## Autres dispositifs OPCO Santé activables

### Bonus thématiques 2026

En plus du PDC standard, OPCO Santé peut majorer le financement sur certaines thématiques prioritaires :

- **FMB HP — Axe Numérique**
- **FMB HP — Axe IA**

## Analyse détaillée — Hospitalisation Privée et Thermalisme

Cette section restitue notre analyse complète des critères de financement publiés officiellement par OPCO Santé pour la branche Hospitalisation Privée et Thermalisme. Données sourcées, vérifiées et mises à jour régulièrement.

## Présentation de la branche

La branche **Hospitalisation Privée et Thermalisme (HP)** regroupe les cliniques privées, les établissements de soins privés à but lucratif et les établissements thermaux. Elle est couverte par OPCO Santé avec une synthèse PDF dédiée mise à jour annuellement.

La contribution conventionnelle FMB est requise pour accéder aux fonds de branche.

## FMB HP : IA et Numérique explicitement financés

Le **Fonds Mutualisé de Branche HP** comprend deux axes directement pertinents pour les OF numériques :
- **Axe "Numérique"** — cofinancement FMB
- **Axe "IA"** — **programme IA explicitement mentionné** comme axe de cofinancement prioritaire

Il s'agit d'un des rares cas dans le panorama des 11 OPCO où l'IA est nommée explicitement comme axe de financement. Les **montants ne sont pas publiés** dans la synthèse 2026 — ils font l'objet d'un dossier à soumettre à OPCO Santé (cofinancement sur devis/projet).

Les autres axes FMB HP couverts : Prévention des risques, QVT, Écologie.

**Qualifications prioritaires FMB HP** (financement majoré) : IBODE, Cuisine, DEAS, DEAES, DEI, Imagerie, Pharmacie, VAE hors plateforme.

## Tutorat : structure spécifique HP

Contrairement aux autres OPCO (qui versent une aide mensuelle sur 4-6 mois), OPCO Santé HP utilise un **forfait global versé après 6 mois** :
- 1 salarié encadré : **500€**
- 2 salariés encadrés : **750€**

## IDCC couvert

**IDCC 2264** — Convention collective nationale de l'hospitalisation privée du 18 avril 2002, portée côté employeurs par la **FHP** (Fédération de l'hospitalisation privée). Confirmé sur Légifrance et code.travail.gouv.fr (2026-05-28).

Le secteur **thermalisme** (ex-IDCC 2104) a vu son champ d'application **fusionné dans l'IDCC 2264 en mars 2019** — d'où le libellé « Hospitalisation Privée et Thermalisme » de la synthèse OPCO Santé. La fiche couvre donc **2264 ET 2104** : bien que 2104 soit juridiquement absorbé, des établissements thermaux continuent de le déclarer en paie/DSN, et le rattacher ici garantit qu'un prospect thermal soit correctement reconnu par le simulateur. Source légale unique du groupement : la fusion des champs conventionnels (Légifrance KALICONT000005635813).

## Signaux commerciaux pour OF tiers

1. **FMB axe IA** → voie la plus directe pour un OF IA dans le secteur HP. Dossier à monter avec OPCO Santé pour valoriser les montants disponibles.
2. **FMB axe Numérique** → formations numériques plus larges (non IA spécifiquement) couvertes dans le même fonds.
3. **PDC <50 sal** → 2 500€/an standard pour les formations classiques.
4. **ContratPro** → 13-18€/h selon public, notamment pour les qualifications paramédicales.

## Sources officielles

Toutes les données chiffrées de cette page proviennent des publications officielles de OPCO Santé et des sources gouvernementales, vérifiées à la date indiquée.

- [https://www.opco-sante.fr/app/uploads/2026/05/opco0275_synthese-des-prise-en-charge-hp_pdf_site.pdf](https://www.opco-sante.fr/app/uploads/2026/05/opco0275_synthese-des-prise-en-charge-hp_pdf_site.pdf) (consulté le 2026-05-18)
- [https://code.travail.gouv.fr/convention-collective/2264-hospitalisation-privee](https://code.travail.gouv.fr/convention-collective/2264-hospitalisation-privee) (consulté le 2026-05-28)

## Pour aller plus loin

- 🎯 **OPCO parent** : [Tous les dispositifs OPCO Santé 2026](/simulateur-opco/opco-sante/)
- 📊 **Actions collectives** : [Formations 100% financées par votre OPCO](/simulateur-opco/actions-collectives/)
- 📖 **Guide complet** : [Financer la formation IA via OPCO 2026](/blog/dispositifs-opco-2026-financer-formation-ia-pme/)

## Calculez votre budget Hospitalisation Privée et Thermalisme 2026

Notre simulateur identifie automatiquement votre rattachement à la branche Hospitalisation Privée et Thermalisme depuis votre raison sociale ou votre numéro SIREN. Aucune connaissance de votre IDCC n'est requise. Le calcul prend 30 secondes et reste sans engagement.

[**Lancer le simulateur OPCO 2026 →**](/simulateur-opco/)
