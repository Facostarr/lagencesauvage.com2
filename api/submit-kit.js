...
// =============================================================================
// Capture prénom + email → Notion + email kit prospect + notification Franck
...
export default async function handler(req, res) {
  ...
  // 2. Email prospect — indépendant de la notification Franck
  try {
    const kitPdfUrl = 'https://www.lagencesauvage.com/assets/downloads/kit-10-prompts-claude-pme.pdf';
...
<div style="background:#fff;border:1px solid #E2E8F0;border-top:none;padding:32px;border-radius:0 0 8px 8px">
  ...
  <a href="${kitPdfUrl}" style="display:inline-block;background:#4F46E5;color:#fff;padding:12px 28px;border-radius:6px;text-decoration:none;font-weight:600;font-size:15px">Télécharger mon kit (PDF) →</a>
  ...
  <p style="margin:0;font-size:12px;color:#94A3B8">L'Agence Sauvage · Paris · <a href="mailto:hello@lagencesauvage.com" style="color:#94A3B8">hello@lagencesauvage.com</a></p>
    ...
    console.log('✅ Email kit PDF envoyé au prospect');
  } catch (err) {
    console.error('⚠️ Email prospect échoué (non bloquant):', err.message);
  }

  // 3. Notification Franck — indépendante de l'email prospect
  try {
    await notifyFounder({
      firstName,
      email,
  ...
  // 4. Tracking Plausible — non bloquant
  try {
